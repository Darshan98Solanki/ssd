    ...
    if (!parseData.success) {
        ...
        conn.query(getUserData, [organization, userId], (err, customerData) => {
            if (err) {
                ...
                const query = "SELECT p.fat,p.purchase_date,p.amount,p.litre,p.milk_type,p.when_,DATE_FORMAT(p.purchase_time , '%Y-%m-%d %r') AS formatted_purchase_date FROM customers c INNER JOIN purchase p ON c.customer_id = p.customer_id AND p.payment_status != 'paid' AND c.organization = ? AND c.user_id = ?;"
